...
export const getMembershipByUserId = async (req, res) => {
    try {
        ...
        if (!userId) {
            return res.status(400).json({ success: false, statusCode: 400, message: "userId is required", data: null })
        }
        ...
        return res.status(500).json({ success: false, statusCode: 500, message: "Error fetching membership", data: null })
    }
}
...
export const renewMembership = async (req, res) => {
    try {
        const { userId, planType, startDate, totalAmount, amountPaid, durationDays: customDuration } = req.body

        if (!userId || !planType) {
            return res.status(400).json({ success: false, statusCode: 400, message: "userId and planType are required", data: null })
        }
        ...
        const newMembership = await Membership.create({
            user_id: userId, planType, durationDays,
            startDate: start, endDate: end, rolloverDays,
            attendanceUsed: 0, status: "ACTIVE",
            totalAmount: parsedTotal, amountPaid: parsedPaid, dueAmount, paymentStatus
        })

        return res.status(201).json({
            success: true, statusCode: 201,
            message: `Membership renewed successfully${rolloverDays > 0 ? ` with ${rolloverDays} carry-over days` : ""}`,
            data: { membership: newMembership, rolloverDays, previousMembershipId: currentMembership?._id || null }
        })
    } catch (error) {
        console.error("Error renewing membership:", error)
        return res.status(500).json({ success: false, statusCode: 500, message: "Error renewing membership", data: null })
    }
}
