...
export const toggleAttendance = async (req, res) => {
    ...
    try {
        if (!memberId || !date) {
            return res.status(400).json({ success: false, message: "memberId and date are required", data: null });
        }

        const membership = await Membership.findById(memberId);
        if (!membership) {
            return res.status(404).json({ success: false, message: "Membership not found", data: null });
        }
        ...
        if (existing) {
            ...
            // Update attendance count on membership
            if (newStatus === "ABSENT") {
            ...
            return res.status(200).json({ success: true, message: `Attendance toggled to ${newStatus}`, data: existing });
        } else {
            ...
            return res.status(201).json({ success: true, message: "Attendance marked as PRESENT", data: attendance });
        }
    }
    catch (error) {
        console.error("Error toggling attendance:", error);
        return res.status(500).json({ success: false, message: "Error toggling attendance", data: null });
    }
}
...
// @access  Admin
// @query   memberId - the membership ID
export const getAttendanceByCycle = async (req, res) => {
    const { memberId } = req.query;

    try {
        if (!memberId) {
            return res.status(400).json({ success: false, message: "memberId is required", data: null });
        }
        ...
        // Fetch all attendance records for this membership
        ...
        for (let i = 0; i < totalDays; i++) {
            ...
            if (dayDate > today) {
                ...
                status = attendanceMap[dateKey]; // PRESENT or ABSENT
            } else {
                status = "ABSENT";
            }
        ...
        return res.status(200).json({
            ...
            data: {
                ...
                cycleInfo: {
                    ...
                    dayInCycle,
                    daysLeft,
                    sessionsCompleted: attendanceUsed,
        ...
        return res.status(500).json({ success: false, message: "Error fetching attendance cycle", data: null });
    }
}
